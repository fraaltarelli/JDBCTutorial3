...
<body>
   ...
   Cerca Città: <input type="text" name="cercacitta" size="10px"
				...
				<c:if test= "${targetservlet != 'runContinenti'}">
				<a href="${targetservlet}">
				<button> indietro
				</button> <br>
				</a>
				</c:if>
  ...
  <c:otherwise>
   ...
   <td width="22%">
   <input name="nomecittainserito"  type="text" value="<c:if test="${cittaDaModificare != null}">
${cittaDaModificare.nomeCitta}</c:if>">  
   </td>
   
   <td width="22%">
   <input name="nomedistrettoinserito" type="text"  value="<c:if test="${cittaDaModificare.distretto != null && currentCityId!=0}">
${cittaDaModificare.distretto}</c:if>">  
   </td>
   <td width="22%">
   <input name="popolazioneinserita" type="number"  min="0" max="300000000000"value="<c:if test="${cittaDaModificare.popolazione != null && currentCityId!=0}">${cittaDaModificare.popolazione}</c:if>">  
   </td>
     ...
     <form action="">
     <c:if test= "${listaCitta != null && listaCittaVuota==false}">
         <table  width="75%" border="1" align="center">
   ...
   <c:if test="${currentList=='Continenti' }">
   <c:forEach items="${listaContinenti}" var = "continente" >
         <a href="runNazioni?aid=${continente}">
         ...
         <c:if test= "${listaNazioni != null && currentList=='Nazioni'}">
         <table  width="75%" border="1" align="center">
    ...
    <c:forEach items="${listaNazioni}" var = "nazione" >
         <a href="runCitta?aid=${nazione.code}&name=${nazione.nomeNazione}">
         <table  width="75%" border="1" align="center">
         <tr>
